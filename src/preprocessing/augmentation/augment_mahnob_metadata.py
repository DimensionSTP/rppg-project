...
def augment_mahnob_metadata(
    ...
    df[config.frame_index_column_name] = df[config.frame_index_column_name].astype(int)
    df[config.frame_rate_column_name] = df[config.frame_rate_column_name].astype(float)
    ...
    for dir_path, group in tqdm(dir_groups):
        for start_index in range(
            ...
            row = {
                ...
                config.frame_rate_column_name: subset_df[
                    config.frame_rate_column_name
                ].max(),
