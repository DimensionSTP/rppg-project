...
def main(
    ...
    for file in file_name:
        ...
        ppg_df = pd.read_csv(f"{data_path}/{file[:-4]}.txt", header=None, delimiter="\t", skiprows=23)
        print("-" * 200)
        ...
        for i in range(0, len(ppg_value) - window_size * sampling_rate, int(interval_size * sampling_rate)):
        ...
        df.columns = ["BPM"]
        df.to_csv(f"{save_path}/{file[:-4]}_300.csv", encoding="utf-8-sig", index=False) #결과 저장
