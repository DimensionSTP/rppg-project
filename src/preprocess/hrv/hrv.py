...
class HRV:
    ...
    def ppi_correction(self, ppi):
        ...
        for i in range(len(ppi)):
            if (ppi[i] >= 60000.0 / 240) and (
                ppi[i] < 60000.0 / 40
            ):  # 30 ~ 180 bpm Normal range 벗어나면 버림
                _nni.append(ppi[i])
            else:
                pass
        """
        ...
        for k in range(len(_nni)):
            if k < 4:  # 처음 4개까지는 그대로 정상 nni 취급
                ...
                if (_nni[k] < mean_b4 * 0.8) or (
                    _nni[k] > mean_b4 * 1.3
                ):  # 이전 ppi 4개 평균의 30%보다 작거나 130%보다 크면 이전 정상 nni의 4개 평균값으로 대체
                    _nni[k] = mean_b4
                    nni.append(_nni[k])
    ...
    def calc_fft(self, signal):
        ...
        # power spectrum
        power_spectrum = (
            ((frequency.real * frequency.real) + (frequency.imag * frequency.imag))
            / (len(frequency) * len(frequency))
            * 2
        )
    ...
    def calc_p(self, power_spectrum, df):
        ...
        peak = power_spectrum[
            peak_index - interval_index : peak_index + interval_index + 1
        ]
